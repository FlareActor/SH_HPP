...
def load_data():
    """加载训练数据"""
    print('加载挂牌训练数据...')
    cache_path = './cache/guapai_%s-%s.hdf' % (beginDate, endDate)
...
def preprocess(all_df):
    """特征预处理"""
    print('清洗挂牌数据...')
    cache_path = './cache/feats_%s-%s.hdf' % (beginDate, endDate)
    if os.path.exists(cache_path):
        all_df = pd.read_hdf(cache_path, 'data')
    else:
        # 修正面积
...
def make_train_test_set(all_df, test_df=None, meta_df=None):
    ...
    # 测试集
    x_test = test_df.drop(['address', 'name', 'house_type'], axis=1)
...
if __name__ == '__main__':
    ...
    '''模型训练'''
    # * LightGBM
    cache_path = './cache/model_%s-%s.txt' % (beginDate, endDate)
    if os.path.exists(cache_path):
        ...
        gbm = lgb.train(params, lgb_train, num_boost_round=800)
